...
function mix_board() {
	for (var i = 0; i < Math.pow(table_size, 4); i++) {
		var part_sequence_number = Math.floor((Math.random() * Math.pow(table_size, 2)) + 1);
		var part = $("#part"+part_sequence_number)[0]
		if (part) {
			console.log(part);
			console.log(part.onclick);
		}
		if (part && (part.onclick != null || part.onclick != undefined)) {
			swap_slot("part"+part_sequence_number);
		}
	}
}